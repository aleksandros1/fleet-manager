...
const GlobalStyles = () => (
  <style jsx global>{`
    ...
    /* ΑΛΕΞΙΣΦΑΙΡΟ STICKY BAR - Παρακάμπτει τα σφάλματα του Tailwind JIT */
    .sticky-category-bar {
      position: -webkit-sticky;
      position: sticky;
      top: 73px; /* Ακριβώς κάτω από το mobile header (72px + 1px border) */
      z-index: 30;
    }
    @media (min-width: 768px) {
      .sticky-category-bar {
        top: 81px; /* Ακριβώς κάτω από το desktop header (80px + 1px border) */
      }
    }
  `}</style>
);
...
export default function PremiumFleetApp() {
  ...
  const scrollToSection = (id: string) => {
    ...
    if (element) {
      // Αυξήθηκε το offset στα 135px για να μην κρύβονται τα αυτοκίνητα κάτω από τη μπάρα!
  ...
  const handleAvailabilityClick = (type: string) => {
    setActiveAvailability(type);
    setTimeout(() => {
  ...
  const handleBookingToStripe = async () => {
    ...
    if (activeAvailability === 'Ενοικίαση' && (!selectedRange.start || !selectedRange.end)) return;
    ...
    try {
      ...
      const totalDays = activeAvailability === 'Ενοικίαση' ? getCalculatedDays() : 1;
      ...
      const response = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({
          ...
          model: `${selectedVehicle.model} [${bookingTypeLabel}] ${stripeDescriptionModifier}`,
          price: amountToPay,
          checkIn: checkInDate,
  ...
  const DateRangePicker = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (isPast || isBooked) {
          ...
          dayClass += " bg-[#8B0000] text-white font-bold scale-110";
        } else if (selectedRange.start && selectedRange.end && dateStr > selectedRange.start && dateStr < selectedRange.end) {
          dayClass += " bg-[#8B0000]/15 text-[#8B0000]";
        } else {
          dayClass += " text-gray-300 hover:bg-[#8B0000]/20 hover:text-white cursor-pointer";
        }
    ...
    return (
        <div className="w-full bg-[#111111] p-5 md:p-6 rounded-[2rem] border border-white/5">
          <div className="flex justify-between items-center mb-6">
            <button onClick={() => setCalendarDate(new Date(year, month - 1, 1))} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-[#8B0000] text-gray-400 hover:text-white transition-colors"></button>
    ...
    <div className="min-h-screen bg-[#030303] text-white selection:bg-[#8B0000]/40 relative">
      ...
      <div className={`fixed inset-0 z-[200] bg-[#050505] flex flex-col transition-all duration-500 ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
        <div className="px-5 md:px-12 py-4 flex justify-between items-center border-b border-white/5">
          ...
          <button onClick={() => setIsMenuOpen(false)} className="p-2 text-white hover:text-[#8B0000] transition-colors">
        ...
        <div className="pb-12 flex justify-center items-center gap-8">
          <a href="https://instagram.com" target="_blank" rel="noreferrer" className="text-gray-400 hover:text-[#8B0000] transition-colors">
          ...
          <a href="tel:+306948766884" className="text-gray-400 hover:text-[#8B0000] transition-colors">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
          </a>
        </div>
      </div>

      <header className="fixed top-0 w-full z-40 bg-black border-b border-[#8B0000]/20 px-5 md:px-12 py-4 flex justify-between items-center transition-all">
        <AutoLazaridisLogo className="h-10 md:h-12 w-auto" />
        
        <div className="flex items-center gap-6">
          <button 
            onClick={toggleLanguage}
            className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest bg-[#111] hover:border-[#8B0000] px-3 py-2 rounded-lg transition-colors border border-white/10"
          >
          ...
          <button onClick={() => setIsMenuOpen(true)} className="p-2 text-white hover:text-[#8B0000] transition-colors focus:outline-none">
      ...
      <section id="home" className="relative w-full h-[50vh] md:h-[65vh] flex flex-col justify-center items-center text-center px-5 md:px-12 mt-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-500 w-full px-2 mt-4">
          ...
          <p className="text-[11px] md:text-sm text-gray-400 max-w-xl mx-auto font-light leading-relaxed tracking-wide px-4 mb-8">
            {t.heroSub}
          </p>
          
          {/* Σταθερό Πλέγμα (Grid) για τα τρία βασικά κουμπιά - Χωρίς Overflow, απόλυτα συμμετρικά */}
          <div className="w-full max-w-[320px] md:max-w-md mx-auto bg-[#111]/80 backdrop-blur-md p-1.5 md:p-2 rounded-full border border-white/10 grid grid-cols-3 gap-1 shadow-2xl shadow-[#8B0000]/10">
            {['Ενοικίαση', 'Leasing', 'Πώληση'].map(type => (
              <button 
                key={type}
                onClick={() => handleAvailabilityClick(type)}
                className={`w-full py-3 md:py-3.5 rounded-full text-[8px] md:text-[10px] font-bold uppercase tracking-wider transition-all duration-300 ${activeAvailability === type ? 'bg-[#8B0000] text-white shadow-[0_0_15px_rgba(139,0,0,0.4)]' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
              >
      ...
      {/* Νέα Κλάση .sticky-category-bar αντί για tailwind */}
      <div className="sticky-category-bar bg-[#030303]/95 backdrop-blur-xl px-5 md:px-12 py-3 md:py-4 flex justify-start md:justify-center gap-2 md:gap-3 overflow-x-auto hide-scrollbar border-b border-[#8B0000]/10">
        {categories.map(cat => (
          <button 
            key={cat} 
            onClick={() => {
            ...
            className={`flex-shrink-0 text-[10px] md:text-xs font-bold uppercase tracking-widest px-5 py-3 rounded-full transition-all duration-300 ${activeCategory === cat ? 'bg-[#8B0000] text-white' : 'bg-[#111] text-gray-400 hover:bg-white/10 hover:text-white border border-white/5'}`}
          >
            {cat === 'All' ? t.all : cat}
          </button>
        ))}
      ...
      <main id="fleet" className="px-5 md:px-12 py-16 md:py-20 max-w-[1400px] mx-auto pb-safe min-h-[40vh]">
        {loading ? (
          <div className="text-center py-32 text-[#8B0000] text-[10px] md:text-sm uppercase tracking-widest animate-pulse">{t.updating}</div>
        ) : displayedVehicles.length === 0 ? (
          /* Empty State - Αποτρέπει τη "μαύρη τρύπα" */
          <div className="flex flex-col items-center justify-center py-20 md:py-32 opacity-70">
            <svg className="w-12 h-12 text-[#8B0000]/50 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
            <div className="text-gray-400 text-[10px] md:text-sm uppercase tracking-widest text-center font-bold">ΔΕΝ ΒΡΕΘΗΚΑΝ ΟΧΗΜΑΤΑ ΓΙΑ ΑΥΤΗ ΤΗΝ ΕΠΙΛΟΓΗ.</div>
          </div>
        ) : (
          <div className="flex flex-col gap-10 md:gap-12">
            {displayedVehicles.map(v => (
              <div key={v.id} className="group flex flex-col md:flex-row bg-[#0A0A0A] border border-white/5 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden hover:border-[#8B0000]/40 transition-all duration-500 shadow-lg hover:shadow-[0_0_30px_rgba(139,0,0,0.1)]">
                ...
                <div className="w-full md:w-1/2 p-6 md:p-16 flex flex-col justify-center relative z-10">
                  <div className="inline-block px-3 md:px-4 py-1 md:py-1.5 rounded-full bg-[#8B0000]/10 text-[#8B0000] text-[8px] md:text-[9px] font-bold uppercase tracking-widest w-fit mb-4 md:mb-6 border border-[#8B0000]/20">{v.category}</div>
                  <h2 className="text-3xl md:text-5xl font-serif-premium font-light mb-4 md:mb-6 leading-tight group-hover:text-[#8B0000] transition-colors">{v.model}</h2>
                  
                  <div className="flex gap-4 md:gap-8 mb-8 md:mb-10 pb-8 md:pb-10 border-b border-white/5 group-hover:border-[#8B0000]/20 transition-colors">
                    <div>
                      <div className="text-[9px] md:text-[10px] text-[#8B0000]/70 uppercase tracking-widest mb-1">{t.engine}</div>
                    ...
                    <div>
                      <div className="text-[9px] md:text-[10px] text-[#8B0000]/70 uppercase tracking-widest mb-1">{t.power}</div>
                    ...
                    <div>
                      <div className="text-[9px] md:text-[10px] text-[#8B0000]/70 uppercase tracking-widest mb-1">{t.transmission}</div>
                  ...
                  <div className="mt-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-8">
                    <div>
                      ...
                      <div className="text-2xl md:text-4xl font-light">
                        €{v.price}
                        {activeAvailability === 'Ενοικίαση' && <span className="text-xs md:text-sm text-[#8B0000] ml-2">{t.perDay}</span>}
                        {activeAvailability === 'Leasing' && <span className="text-xs md:text-sm text-[#8B0000] ml-2">{t.perMonth}</span>}
                      </div>
                    </div>
                    <button onClick={() => setSelectedVehicle(v)} className="w-full md:w-auto px-8 md:px-10 py-4 md:py-5 bg-white text-black rounded-full text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-[#8B0000] hover:text-white transition-all duration-300 shadow-md hover:shadow-[0_0_20px_rgba(139,0,0,0.5)]">
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex flex-col md:flex-row gap-10 md:gap-16 mb-16">
          ...
          <div className="w-full md:w-1/3 flex flex-col justify-start items-start md:items-start text-left gap-6 md:gap-8">
            ...
            <div className="flex flex-col gap-4 text-[10px] md:text-xs uppercase tracking-widest text-gray-400">
              <div>
                <div className="text-[#8B0000] mb-1">Τοποθεσια</div>
                <span className="text-white">{t.address}</span>
              </div>
              <div>
                <div className="text-[#8B0000] mb-1">Τηλεφωνο</div>
          ...
          <div className="w-full md:w-2/3 h-64 md:h-80 rounded-2xl overflow-hidden border border-white/10 grayscale-[50%] hover:grayscale-0 transition-all duration-500 hover:border-[#8B0000]/30 shadow-lg hover:shadow-[0_0_15px_rgba(139,0,0,0.2)]">
            <iframe 
              src="https://maps.google.com/maps?q=41.1145067,24.2068323&hl=el&z=16&output=embed" 
              width="100%" 
              height="100%" 
              style={{ border: 0 }} 
          ...
          <div className="relative w-full md:w-[550px] h-[100dvh] bg-[#0A0A0A] md:border-l border-[#8B0000]/20 flex flex-col animate-in slide-in-from-bottom md:slide-in-from-right duration-500 md:rounded-l-[3rem] overflow-hidden shadow-[-20px_0_50px_rgba(139,0,0,0.1)]">
            ...
            <div className="px-5 md:px-8 py-5 border-b border-white/5 flex justify-between items-center bg-[#050505] z-20 shrink-0">
              <button 
                ...
                className="flex items-center gap-3 text-white hover:text-[#8B0000] transition-colors group"
              >
                <div className="w-8 h-8 flex items-center justify-center rounded-full bg-[#111] border border-white/10 group-hover:border-[#8B0000]/50 group-hover:bg-[#8B0000]/10 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto pb-safe px-5 md:px-8 pt-6 space-y-8 hide-scrollbar">
              ...
              {/* Εμφάνιση Ημερολογίου ΜΟΝΟ στην Ενοικίαση */}
              ...
              <div className="bg-[#111] border border-white/5 p-6 md:p-8 rounded-[2.5rem] hover:border-[#8B0000]/30 transition-colors">
                <div className="mb-5">
                  <h4 className="text-[10px] text-[#8B0000] uppercase tracking-[0.2em] font-bold mb-1">{t.personalInfoTitle}</h4>
                </div>
                <div className="space-y-4">
                  <input 
                    ...
                    className="w-full bg-black border border-white/10 rounded-xl px-4 py-4 text-sm text-white focus:outline-none focus:border-[#8B0000] focus:shadow-[0_0_10px_rgba(139,0,0,0.2)] transition-all"
                  />
                  <input 
                    type="tel" 
                    required
              ...
              <div className="bg-[#111] border border-white/5 p-6 md:p-8 rounded-[2.5rem] hover:border-[#8B0000]/30 transition-colors">
                <div className="mb-5">
                  <h4 className="text-[10px] text-[#8B0000] uppercase tracking-[0.2em] font-bold mb-1">Fast Track Check-In</h4>
                ...
                <div className="flex gap-3 md:gap-4">
                  <button onClick={() => idInputRef.current?.click()} className={`flex-1 py-3 md:py-4 rounded-xl text-[9px] uppercase tracking-widest border transition-all ${idFile ? 'border-[#8B0000] bg-[#8B0000]/10 text-[#8B0000] shadow-[0_0_15px_rgba(139,0,0,0.2)]' : 'border-white/10 text-gray-400 hover:bg-white/5 hover:border-[#8B0000]/50'}`}>
                    {idFile ? '✓ ' + idFile.name.substring(0, 8) + '...' : t.idCard}
                  </button>
                  <button onClick={() => licenseInputRef.current?.click()} className={`flex-1 py-3 md:py-4 rounded-xl text-[9px] uppercase tracking-widest border transition-all ${licenseFile ? 'border-[#8B0000] bg-[#8B0000]/10 text-[#8B0000] shadow-[0_0_15px_rgba(139,0,0,0.2)]' : 'border-white/10 text-gray-400 hover:bg-white/5 hover:border-[#8B0000]/50'}`}>
              ...
              {showPaymentSection && (
                <div className="relative bg-[#111] border border-[#8B0000]/30 rounded-[2.5rem] p-6 md:p-8 overflow-hidden mt-8 shadow-[0_0_30px_rgba(139,0,0,0.1)]">
                  ...
                  <div className="relative z-10">
                    <h3 className="text-[10px] uppercase tracking-[0.3em] text-[#8B0000] mb-6 font-bold">
                      {activeAvailability === 'Ενοικίαση' 
                        ? `${t.rentPaymentInfo} (${getCalculatedDays()} ${t.daysSelected || 'ΗΜΕΡΕΣ'})` 
                        : activeAvailability === 'Leasing' 
                          ? t.leasePaymentInfo 
                          : t.buyPaymentInfo}
                    </h3>
                    ...
                    <div className="flex gap-3 md:gap-4 mb-8">
                      <div 
                        ...
                        className={`flex-1 p-4 md:p-5 rounded-2xl border cursor-pointer transition-all duration-300 ${paymentMode === 'full' ? 'bg-[#8B0000]/10 border-[#8B0000] shadow-[0_0_15px_rgba(139,0,0,0.2)]' : 'border-white/10 hover:border-[#8B0000]/50 bg-black/20'}`}
                      >
                        <div className={`text-[9px] uppercase tracking-widest mb-2 transition-colors ${paymentMode === 'full' ? 'text-[#8B0000]' : 'text-gray-500'}`}>
                          {t.fullPayment}
                        </div>
                      ...
                      <div 
                        ...
                        className={`flex-1 p-4 md:p-5 rounded-2xl border cursor-pointer transition-all duration-300 ${paymentMode === 'deposit' ? 'bg-[#8B0000]/10 border-[#8B0000] shadow-[0_0_15px_rgba(139,0,0,0.2)]' : 'border-white/10 hover:border-[#8B0000]/50 bg-black/20'}`}
                      >
                        <div className={`text-[9px] uppercase tracking-widest mb-2 transition-colors ${paymentMode === 'deposit' ? 'text-[#8B0000]' : 'text-gray-500'}`}>
                          {t.deposit}
                        </div>
                    ...
                    <button 
                      ...
                      className={`w-full py-5 rounded-full text-[10px] font-bold tracking-[0.2em] uppercase transition-all duration-300 shadow-[0_0_20px_rgba(139,0,0,0.3)] ${isSubmitting ? 'bg-white/5 text-gray-600 cursor-not-allowed' : 'bg-[#8B0000] text-white hover:bg-[#6A0000] hover:scale-[1.02]'}`}
                    >
